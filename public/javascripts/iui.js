	...
	if (inProgress==false) {
	    ...
	    if (link)
	    {
	        if (link == $("backButton")) {
				...
				inProgress=true;
				setTimeout("inProgress=false",1);
				
				z = WindowHistory.pop()
				h = window.location.href
				
				if (h.indexOf('blip_categories/1/blips/new#_edit') != -1) {
...
function updatePage(page, fromPage)
{
    ...
    if (backButton)
    {
		...
		//	switchmenu("off")
			...
			if ($(pageHistory[pageHistory.length-2]) == null) {
				//	switchmenu("on")
			}
...
function slidePages(fromPage, toPage, backwards)
{
	if (mobile_format == 'iPhone') {
		...
		tempfrom=fromPage;
	    setTimeout("tempfrom.removeAttribute('selected')",500);
	    checkTimer = setInterval(checkOrientAndLocation, 300);
